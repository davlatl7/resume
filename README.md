<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/1830837e-1a61-4148-ae07-8b60460f6256" />﻿# Давлат Эркабоев - Golang Developer

- **Phone:** +992 008 80 04 45
- **Email:** [davlat7747@gmail.com](mailto:davlat7747@gmail.com)
- **Github:** [github.com/davlatl7](https://github.com/davlatl7)
- **LinkedIn:** [linkedin.com/in/давлат-93b942331/](https://www.linkedin.com/in/давлат-93b942331/)

## About Me
Motivated and results-oriented software developer with a strong focus on building clean, scalable, and reliable backend applications. Skilled at working both independently and as part of a collaborative team. Committed to continuous learning and capable of turning complex challenges into opportunities for innovation and growth

## Work Experience

 ### Apartment Rental
**Golang Developer**  
_May 2024 – September 2024_  
Stack: Golang, PostgreSQL  

### Bus Depot — Golang Developer
**Golang Developer** 
_May 2025 – July 2025_
Stack: Go, PostgreSQL
 • Built a full backend system for managing bus routes, drivers, ticketing, and schedules.
 • Implemented functionality similar to the Apartment Rental project, including CRUD operations, authentication, structured API design, and database modeling.
 • Ensured clean architecture, modular code organization, and maintainable project structure.

- Completed a comprehensive GO Developer (Junior+) course, culminating in a feature-rich final project titled "Apartment Rental."
- Completed a comprehensive Go Developer (Junior+) course delivered by Minds.io and DushanbeCity.

  Work experience through personal projects: 1 year+

### Sport Programming
**C++, Python Developer**  
_September 2022 – Present_  
Stack: C++, Python


## Technical Skills

- **Backend:** Golang, Python
- **Frameworks:** Gin, Gorm
- **Databases:** PostgreSQL, MySQL
- **Tools:** Docker, RestAPI
- **Frontend:** HTML, CSS

## Education
**The Branch of Moscow State University in Dushanbe**  
Currently pursuing a Bachelor of Science in Applied Mathematics and Informatics  
4rd year student (2022 - 2026)




